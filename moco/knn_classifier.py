...
def knn_classifier(X1, X2, Y1, n_sample=20000, knn=10):
    X1 = X1.copy()
    X2 = X2.copy()
    Y1 = Y1.copy()
...
def knn_classifier_top_k(X1, X2, Y1, n_sample=20000, knn=10, top_k=10):
    top_k = min(top_k, knn)
    X1 = X1.copy()
    X2 = X2.copy()
    Y1 = Y1.copy()
...
def knn_classifier_prob(X1, X2, Y1, n_sample=None, knn=10, crit='count'):
    ...
    return res  #返回预测的label和置信度

def knn_classifier_prob_stupid(X1, X2, Y1, n_sample=None, knn=10, num_chunks=100):
    '''
    return :
        target_neighbor: predicted label
        traget_prob: confidence score
    ...
    return target_pred, target_prob #返回预测的label和置信度
